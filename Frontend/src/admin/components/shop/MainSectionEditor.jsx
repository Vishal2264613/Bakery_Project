...
const MainSectionEditor = () => {
  const [sectionName, setSectionName] = useState("shopSection");
  const [heading, setHeading] = useState("");
  const [imageUrls, setImageUrls] = useState([]); // ARRAY of images
  ...
  const handleImageUpload = async () => {
    ...
    try {
      ...
      setImageUrls((prev) => [...prev, data.secure_url]); // push to array
      setImageFile(null);
    } catch (err) {
  ...
  const handleSaveHero = async () => {
    ...
    const heroData = {
      heading,
      description,
      image_urls: imageUrls, // ✅ no nesting inside array
    };
