  ...
  return (
    ...
    <section className=' flex justify-center items-center w-[20%] h-screen'>
        ...
        <div className='w-full flex flex-col justify-center items-center'>
          <img src={logoImg} alt="logo" className="w-[30px] h-[30px] cursor-pointer mt-0 mb-6" />
          ...
          <img
            ...
            />
                ...
                <div className="flex flex-col justify-start items-start w-full  max-sm:hidden mb-6">
                  <h1 className='mb-4 text-sm '>MAIN</h1>
                      {dashboardMainList.map((item) => {
                    const path = `/admin/dashboard/${item.title.toLowerCase()}`;
                        return (
                          <h2 key={item.title} className='mb-6 font-bold text-xl tracking-wide cursor-pointer'> <Link to={path}>{item.title}</Link></h2>
                        );
                      })}
                    </div>

                    <div className="flex flex-col justify-start items-start w-full  max-sm:hidden">
                  <h1 className='mb-4 text-sm '>COMPONENTS</h1>
